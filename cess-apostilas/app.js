...
let config = { modelo: MODELO_PADRAO, rodape: RODAPE_PADRAO, chave: '' };
...
function novaAula(dados = {}) {
  projeto.aulas.push({ url: '', titulo: '', texto: '', status: '', ...dados });
  desenharAulas();
  salvar();
}
...
function aulasProntas() {
  return projeto.aulas.filter(a => a.texto && a.texto.trim().length > 50);
}
